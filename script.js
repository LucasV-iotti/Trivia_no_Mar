/* ===============================
   Gato Pirata — v8.4.4 (navio animado + botões com fallbacks)
   =============================== */
...
function renderStage(){
  ...
  attachDnD(); setProgress(); speak("Navio animado pronto. Arraste as fichas e clique em Verificar."); $("#nextBtn").disabled=true;
}
...
function onNext(){ console.log('[BTN] Próxima'); if(state.stageIndex < stages.length-1){ state.stageIndex++; renderStage(); } else { $("#statement").innerHTML = `<strong>Parabéns!</strong> Você concluiu todas as telas. Pontuação final: <strong>${state.score}</strong>.`; $("#options").innerHTML=''; $("#nextBtn").disabled = true; speak('Missão cumprida! Bons ventos!'); } setProgress(); }

window.__game = { onShuffle, onReset, onCheck, onNext };
document.addEventListener('click', (ev)=>{ const id = ev.target && ev.target.id; if(id==='shuffleBtn') onShuffle(); else if(id==='resetBtn') onReset(); else if(id==='checkBtn') onCheck(); else if(id==='nextBtn') onNext(); });
...
async function boot(){ console.log('[BOOT] Iniciando...'); renderStage(); [ ['#shuffleBtn', onShuffle], ['#resetBtn', onReset], ['#checkBtn', onCheck], ['#nextBtn', onNext] ].forEach(([sel,fn])=>{ const el=$(sel); if(el){ el.addEventListener('click', fn); } }); const ok = await loadUserImage(); if(!ok){ speak('Não encontrei a imagem do personagem. Coloque em assets/user_character.png (ou .jpg).'); } redrawCanvas(); speak('Pronto! Navio animado no banner.'); }
